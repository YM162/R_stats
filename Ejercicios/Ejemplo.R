...
make_barchart_race <- function(data,x,y,
  ...
  data %>%
    ...
    #ggplot(aes(x=as.numeric(as.factor(-rank)),y=!!y,fill=!!x, group=!!x)) +
    ...
    coord_flip(clip="off") +
    #scale_x_continuous(breaks = 1:6, labels = unique(data$rank))+
    scale_x_discrete("") +
    scale_y_continuous("",labels=scales::comma)+
    #view_follow(fixed_x = T, fixed_y = F)
...
make_barchart_race(total.acc_smoother,
                   Country.Region,
                   cases,
                   title="Evolución de los contagios totales/n de CoVid-19 en paises seleccionados",
                   caption="",
                   nframes=100,
                   fps=30,
                   end_pause=20)
